		...
		<?php
			try{	
				...
				if($od == "regi"){
					...
					/*insert menber_list*/
					$sql = 'INSERT INTO member_list(password, name,email, postal1, postal2, address, tel,sex,born) VALUES(?,?,?,?,?,?,?,?,?)';
					$stmt = $dbh->prepare($sql);
					$data = array();
					$data[] = md5($pass);
					$data[] = $onamae;
			...
			if($od == "regi"){
				...
				print "<br/>";
				print "***ご入力の内容で登録が完了しました。***<br/>";
				print "次回からメールアドレスとパスワードでのログインが可能です。<br/>";

			}

			print '<a href = "sh_list.php">top画面へ戻る</a>';
